...
$execute if score $roll_temp expension_dice matches 0 run tellraw @a [{text:"<",color:"green"},{selector:"@s"},{text:"> $(roll)D$(max)="},{score:{name:"$result",objective:"expension_dice"}}]