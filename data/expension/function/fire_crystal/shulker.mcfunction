...
kill @e[type=minecraft:shulker_bullet,distance=..50]
execute as @e[type=minecraft:shulker,distance=..50] run attribute @s max_health modifier add expension:fire_crystal -0.5 add_multiplied_total

advancement revoke @s only expension:check/fire_crystal/shulker